...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ZENX_ROOT"/vendor/zenx/build/tools/smali/baksmali.jar
        export SMALIJAR="$ZENX_ROOT"/vendor/zenx/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ZENX_ROOT"/vendor/zenx/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ZENX_ROOT"/vendor/zenx/build/tools/"$HOST"/compact_dex_converter
    fi
