...
function setup_vendor() {
    ...
    export ZENX_ROOT="$3"
    if [ ! -d "$ZENX_ROOT" ]; then
        echo "\$ZENX_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ZENX_ROOT/$OUTDIR" ]; then
        mkdir -p "$ZENX_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ZENX_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$ZENX_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$ZENX_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ZENX_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ZENX_ROOT"/prebuilts/tools-extras/common/smali/baksmali.jar
        export SMALIJAR="$ZENX_ROOT"/prebuilts/tools-extras/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ZENX_ROOT"/prebuilts/tools-extras/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ZENX_ROOT"/prebuilts/tools-extras/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$ZENX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ZENX_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$ZENX_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ZENX_ROOT"/vendor/zenx/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ZENX_ROOT"/"$OUTDIR"/radio
