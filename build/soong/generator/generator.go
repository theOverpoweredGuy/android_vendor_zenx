...
func init() {
	android.RegisterModuleType("zenx_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zenxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zenxExpandVariables(ctx, dep_file)
	...
	cmd := zenxExpandVariables(ctx, String(g.properties.Cmd))
