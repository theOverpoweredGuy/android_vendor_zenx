function __print_zenx_functions_help() {
cat <<EOF
Additional ZenX-OS functions:
...
- githubremote:    Add git remote for ZenX-OS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ZenX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch zenx_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/zenx-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.zenx.device | grep -q "$ZENX_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $ZENX_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/ZenX-OS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.zenx.device | grep -q "$ZENX_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.zenx.device | grep -q "$ZENX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/zenx/build/tools/repopick.py $@
}
